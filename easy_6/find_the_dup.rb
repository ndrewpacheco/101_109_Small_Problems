...
# compare the orginal_array and array.uniq
# if they don't match, that is the element
# make a counter, if each arrays index has the same value go on
def find_dup(arr)
  uniq_arr = arr.uniq
  counter = 0
  num = 0
  loop do 
    if uniq_arr[counter] == arr[counter]
      counter += 1
    else
      num = arr[counter]
      break

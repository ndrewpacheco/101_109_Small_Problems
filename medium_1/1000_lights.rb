...
def lightswitch(n)
  ...
  switches_array = (1..n).to_a

  switches_hsh = {} 
  switches_array.each {|num| switches_hsh[num] = "off" }


  switches_array.each do |num|
    switches_hsh.each_pair do |key, value|
      if key % num == 0
        if value == "off"
          switches_hsh[key] = "on"
        else
          switches_hsh[key] = "off"
        end
      end
    end
    # binding.pry
  end
switches_hsh.select {|k,v| v == "on"}.keys
end
